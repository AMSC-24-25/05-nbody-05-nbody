...
public:
    explicit NBodyND(std::unique_ptr<Integrator<FP>> integrator, size_t dimensions)
...
void NBodyND<FP>::setup(std::string file_name) {
    ...
    FP current_time = 0.0;
    while (current_time <= 1.0) { // Simula fino al tempo massimo 1.0 (o usa data["max_time"] se disponibile)
...
void NBodyND<FP>::solve() {
    ...
    for (size_t step = 0; step < this->time.size(); ++step) {
        ...
        for (size_t i = 0; i < this->N; ++i) {
            ...
            for (size_t d = 0; d < Dim; ++d) {
                std::cout << this->particles[i].pos[d] << " ";
            }
            std::cout << "\n";
        }
    }
...
// Metodo output
template<std::floating_point FP>
void NBodyND<FP>::output() {
    std::ofstream output_file("nbody_output.csv");

    if (!output_file.is_open()) {
        throw std::runtime_error("Error: Unable to open the output file!");
    }

    std::cout << "Writing output to 'nbody_output.csv'...\n";
    ...
    for (size_t step = 0; step < this->time.size(); ++step) {
        std::cout << "Time step " << this->time[step] << ":\n";
        for (const auto& particle : this->particles) {
            output_file << this->time[step]; // Tempo
            std::cout << "t=" << this->time[step] << " ";

            for (size_t d = 0; d < Dim; ++d) {
                output_file << "," << particle.pos[d];
                std::cout << "x" << d << "=" << particle.pos[d] << " ";
            }
            for (size_t d = 0; d < Dim; ++d) {
                output_file << "," << particle.vel[d];
                std::cout << "v" << d << "=" << particle.vel[d] << " ";
            }
            output_file << "," << particle.mass << "\n";
            std::cout << "m=" << particle.mass << "\n";
        }
    }

    output_file.close();
    std::cout << "Output written to 'nbody_output.csv'\n";
}
